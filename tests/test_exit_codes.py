...
class TestExitCodes(unittest.TestCase):
    ...
    def test_fail_on_low_risk(self, mock_check):
        mock_check.return_value = {
            'schema_version': '1',
            'indicator': {'input': 'https://x', 'type': 'url', 'canonical': 'https://x', 'domain': 'x'},
            'url': 'https://x',
            'domain': 'x',
            'risk_score': 30,
            'verdict': 'LOW_RISK',
        ...
        with patch('sys.argv', ['url-reputation', 'https://x', '--format', 'json', '--fail-on', 'LOW_RISK']):
    ...
    def test_no_fail_on_defaults_zero(self, mock_check):
        ...
        with patch('sys.argv', ['url-reputation', 'https://x', '--format', 'json']):
            with patch('sys.stdout', new=StringIO()):
                with self.assertRaises(SystemExit) as ctx:
                    main()
