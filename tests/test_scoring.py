...
class TestScoringDeterminism(unittest.TestCase):
    def test_deterministic_breakdown_order(self):
        ...
        agg1 = aggregate_risk_score(a, provider_weights={"phishtank": 1.0, "urlhaus": 1.0, "virustotal": 1.0})
        agg2 = aggregate_risk_score(b, provider_weights={"phishtank": 1.0, "urlhaus": 1.0, "virustotal": 1.0})
    ...
    def test_redirects_and_domain_age_rules(self):
        enrichment = {
            "redirects": {
                ...
                "hops": 2,
                "chain": [
                    {"url": "http://start.example", "status": 301, "location": "https://other.example/"},
                    {"url": "https://other.example/", "status": 302, "location": "https://final.example/path"},
        ...
        agg = aggregate_risk_score({}, enrichment=enrichment, provider_weights={"redirects": 1.0, "whois": 1.0})
        ...
        self.assertTrue(any("Redirects observed" in r or "Multiple redirects" in r for r in agg.reasons))
...
class TestCheckerOutputExplainability(unittest.TestCase):
    def test_check_url_includes_breakdown_and_reasons(self):
        ...
        with patch("url_reputation.checker.get_default_registry", return_value=Registry({"phishtank": _DummyProvider()})):
